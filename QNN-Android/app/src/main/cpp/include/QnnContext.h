 ...
 *                    updatable tensors associated with the context and graph combination. Binary sections
 *                    of type QNN_CONTEXT_SECTION_UPDATABLE have Qnn System Context metadata containing
 *                    information about any modified input and output tensors, and therefore may be used
 *                    with QnnSystemContext_getMetadata() and QnnSystemContext_getBinaryInfo().
 *
 * @param[in] profile The profile handle on which metrics are populated and can be queried. Use
 *                    NULL handle to disable profile collection. A handle being re-used would reset
 *                    and is populated with values from the current call.
 ...
 *
 * @note Use corresponding API through QnnInterface_t.
 * @note When using this API with graph switch enabled (non-zero
 *       QNN_CONTEXT_CONFIG_MEMORY_LIMIT_HINT and QNN_CONTEXT_CONFIG_PERSISTENT_BINARY),
