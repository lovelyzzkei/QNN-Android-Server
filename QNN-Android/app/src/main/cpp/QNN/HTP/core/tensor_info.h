...
struct TensorInfo {
    ...
    NameSet early_crouton = 0; // assign crouton during the early approximate phase
...
struct EarlyCrouton : public StringList {
    template <typename... Args> explicit EarlyCrouton(Args... args) : StringList(args...) {}
};
...
struct TensorInfoBuilder : public hnnx::TensorInfo {
    ...
    inline void update1(const EarlyCrouton operand_list)
    {
        const unsigned f = nameset(operand_list, /*input_only*/ true).first;
        early_crouton |= f;
    }
