...
//        }
    ...
    if (StatusCode::SUCCESS != this->createPowerConfigId()) {
        QNN_ERROR("Power Config Id Creation failure");
    }
    QNN_INFO("Power configuration initialized");
...
StatusCode QnnManager::createPowerConfigId() {
    ...
    Qnn_ErrorHandle_t perfInfraErr = perfInfra.createPowerConfigId(m_deviceId, m_coreId, &m_powerConfigId);
...
StatusCode QnnManager::setPowerConfig() {
    QnnDevice_Infrastructure_t deviceInfra = nullptr;
    Qnn_ErrorHandle_t devErr = m_qnnFunctionPointers.qnnInterface.deviceGetInfrastructure(&deviceInfra);
    if (devErr != QNN_SUCCESS) {
        QNN_ERROR("device error");
...
StatusCode QnnManager::setRpcLatencyAndPolling() {
    QnnDevice_Infrastructure_t deviceInfra = nullptr;
